...
Number *Float::add(Number * num) {
	...
	this->set_val(this->value + (double)cast->val());
	delete cast;
	...
	cast->set_val(this->value + cast->real(), cast->imag());
	delete(this);
	delete(cast);
	return result;
    }
...
Number *Float::mul(Number *num) {
	...
	Complex* cast = dynamic_cast<Complex*>(num);
	Complex* result = new Complex(this->value * cast->real(), this->value * cast->imag());
	delete(this);
	delete(cast);
